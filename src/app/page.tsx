...
//     try {
//       const initialResponse = await fetch('/api/search', {
//         method: 'POST',
//         body: formData,
//       });
...
//         throw new Error(errorData.error || 'Failed to start search job.');
//       }
...
//       } else {
//         setError('An unexpected error occurred during job submission.');
//       }
//       setIsLoading(false);
//       console.error(err);
//     }
...
export default function Home() {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
        ...
        const vectorResponse = await fetch(`${pythonApiUrl}/get-vector`, {
            method: 'POST',
            body: formData,
        });

        if (!vectorResponse.ok) {
            throw new Error('The AI server failed to process the image.');
        }
        const userImageVector = await vectorResponse.json();
